...
import { Points } from "../../types/Scene";
...
class Scene extends Phaser.Scene {
  ...
  loadDiamonds(amountOfDiamonds: number) {
    ...
    this.diamondClickHandler();
    this.init_text_labels();
  }

  init_text_labels() {
    this.balance_text = this.add.text(20, 20, "", {
      font: "26px Arial",
      fill: "#00CED1",
      backgroundColor: "white",
    });
    this.dollar_balance_text = this.add.text(
      20,
      50,
      "$ " + this.dollar_balance,
      { font: "26px Arial", fill: "#1E90FF", backgroundColor: "white" }
    );
    let param_style = {
      font: "14px Arial",
      fill: "#66CDAA",
    ...
    this.happiness_text = this.add.text(
      20,
      100,
      "Happiness: " + this.parameters.happiness,
      param_style
    );
    this.sea_life_text = this.add.text(
      20,
      116,
      "Sea life: " + this.parameters.sea_life,
      param_style
    );
    this.plastic_text = this.add.text(
      20,
      132,
      "Plastic: " + this.parameters.plastic,
      param_style
    );
    this.sea_level_text = this.add.text(
      20,
      148,
      "Sea level: " + this.parameters.sea_level,
      param_style
    );
    this.ph_text = this.add.text(
      20,
      160,
      "PH level: " + this.parameters.ph,
      param_style
    );
    this.o2_text = this.add.text(
      20,
      176,
      "O2: " + this.parameters.o2,
      param_style
    );
    this.co2_text = this.add.text(
      20,
      192,
      "CO2: " + this.parameters.co2,
      param_style
    );
    this.nutrition_text = this.add.text(
      20,
      208,
      "Nutrition: " + this.parameters.nutrition,
      param_style
    );
    this.noise_text = this.add.text(
      20,
      224,
      "Noise: " + this.parameters.noise,
      param_style
    );
    this.temp_text = this.add.text(
      20,
      240,
      "Temp: " + this.parameters.temp,
      param_style
    );
  }
  ...
  initialize_swimming_entities() {
    ...
    swimming_entities = [
      ...
      {
        image: this.add
          .image(0, 0, "cruise_ship")
          .setScale(1 / 7)
          .setInteractive()
          .on("clicked", this.dump_plastic),
        curve: new Phaser.Curves.Spline(
  ...
  dump_plastic(g_obj) {
    let image = self
      .get_rundom_plastic_image(g_obj.x, g_obj.y)
      .setScale(1 / 6)
      .setInteractive();
  ...
  get_rundom_swimming_curve(start_x, start_y) {
    const graphics = this.add.graphics();
    graphics.lineStyle(0, 0xffffff, 1);
    return this.drawCurve(start_x, start_y, graphics);
  }

  drawCurve(start_x, start_y, graphics) {
  ...
  generate_points_set(curve1, curve2, modulus) {
    for (let i = 1; i <= this.waves; i += 1) {
      ...
      this.updatePoints(pointsInfo, currentPoint);
    }
  }

  updatePoints(pointsInfo: Points, currentPoint: any) {
    this.points.push(this.get_vector(pointsInfo));
  ...
  get_vector(pointsInfo: Points) {
    if (pointsInfo.modulus % pointsInfo.modulus === 0) {
      return new Phaser.Math.Vector2(
        pointsInfo.midPoint.x + pointsInfo.normal.x * this.vx,
        pointsInfo.midPoint.y + pointsInfo.normal.y * this.vy
      );
    } else {
      return new Phaser.Math.Vector2(
        pointsInfo.midPoint.x +
          pointsInfo.curve1 +
          pointsInfo.normal.x * this.vx,
        pointsInfo.midPoint.y -
          pointsInfo.curve2 +
          pointsInfo.normal.y * this.vy
      );
    }
  }
}
